...
class DataLoader(data.Dataset):
    ...
    def __init__(self, opt):
        ...
        self.split_ix = {'train': [], 'val': [], 'test1': [], 'test2': []}
        for ix in range(len(self.info['images'])):
            ...
            elif img['split'] == 'test1':
                self.split_ix['test1'].append(ix)
            elif img['split'] == 'test2':
                self.split_ix['test2'].append(ix)
            elif opt.train_only == 0:  # restval
                self.split_ix['train'].append(ix)

        print('assigned %d images to split train' % len(self.split_ix['train']))
        print('assigned %d images to split val' % len(self.split_ix['val']))
        print('assigned %d images to split test1' % len(self.split_ix['test1']))
        print('assigned %d images to split test2' % len(self.split_ix['test2']))

        self.iterators = {'train': 0, 'val': 0, 'test1': 0, 'test2': 0}
