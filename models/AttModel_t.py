...
class AttModel_t(CaptionModel):
    def __init__(self, opt):
        super(AttModel_t, self).__init__()
        ...
        if self.use_fc != 0 :
            self.fc_embed = nn.Sequential(nn.Linear(self.fc_feat_size, self.rnn_size),
                                        ...
                                        nn.Dropout(self.drop_prob_lm))
        if self.use_topic != 0 :
            self.topic_embed = nn.Sequential(nn.Linear(self.topic_num, self.input_encoding_size),
                                        nn.ReLU())
    ...
    def forward(self, fc_feats, att_feats, topics, seq):
        ...
        if self.use_topic != 0 :
            topic_feats = self.topic_embed(topics)
        if self.use_fc != 0 :
    ...
    def sample_beam(self, fc_feats, att_feats, topics, opt={}):
        ...
        for k in range(batch_size):
            ...
            if self.use_topic != 0 :
                tmp_topic_feats = topic_feats[k:k+1].expand(beam_size, topic_feats.size(1))
...
class AdaAtt_lstm_t(nn.Module):
    def __init__(self, opt, use_maxout=True):
        super(AdaAtt_lstm_t, self).__init__()
        self.input_encoding_size = opt.input_encoding_size
        #self.rnn_type = opt.rnn_type
        self.rnn_size = opt.rnn_size
        self.num_layers = opt.num_layers
...
class AdaAtt_attention_t(nn.Module):
    def __init__(self, opt):
        super(AdaAtt_attention_t, self).__init__()
        self.input_encoding_size = opt.input_encoding_size
        #self.rnn_type = opt.rnn_type
        self.rnn_size = opt.rnn_size
        self.drop_prob_lm = opt.drop_prob_lm
...
class AdaAttCore_t(nn.Module):
    def __init__(self, opt, use_maxout=False):
        super(AdaAttCore_t, self).__init__()
        self.lstm = AdaAtt_lstm_t(opt, use_maxout)
        self.attention = AdaAtt_attention_t(opt)
...
class TopDownCore_t(nn.Module):
    def __init__(self, opt, use_maxout=False):
        super(TopDownCore_t, self).__init__()
        self.drop_prob_lm = opt.drop_prob_lm

        self.att_lstm = nn.LSTMCell(opt.input_encoding_size + opt.rnn_size * 2, opt.rnn_size) # we, fc, h^2_t-1
        self.lang_lstm = nn.LSTMCell(opt.rnn_size * 2, opt.rnn_size) # h^1_t, \hat v
        self.attention = Attention_t(opt)
...
class Att2in2Core_t(nn.Module):
    def __init__(self, opt):
        super(Att2in2Core_t, self).__init__()
        self.input_encoding_size = opt.input_encoding_size
        #self.rnn_type = opt.rnn_type
        self.rnn_size = opt.rnn_size
        #self.num_layers = opt.num_layers
...
class AdaAttModel_t(AttModel_t):
    def __init__(self, opt):
        super(AdaAttModel_t, self).__init__(opt)
        self.core = AdaAttCore_t(opt)

# AdaAtt with maxout lstm
class AdaAttMOModel_t(AttModel_t):
    def __init__(self, opt):
        super(AdaAttMOModel_t, self).__init__(opt)
        self.core = AdaAttCore_t(opt, True)

class Att2in2Model_t(AttModel_t):
    def __init__(self, opt):
        super(Att2in2Model_t, self).__init__(opt)
        self.core = Att2in2Core_t(opt)
        delattr(self, 'fc_embed')
        self.fc_embed = lambda x : x

class TopDownModel_t(AttModel_t):
    def __init__(self, opt):
        super(TopDownModel_t, self).__init__(opt)
        self.num_layers = 2
        self.core = TopDownCore_t(opt)
